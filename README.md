...
助手只处理「按预算在奥克兰挑 suburb」，别的一律拒答 —— 不是半懂不懂地answer，
而是直接说不做，并给出它能做的例子。

两层执行：页面里一套规则（即时反馈，BYO-key 模式下也生效），代理里同一套规则
（**权威的那层** —— 会滥用开放模型端点的人，正是会改客户端 JS 的人）。
接了模型时，系统提示里也有同一条规则，模型返回 `on_topic:false` 同样触发拒答。

判定刻意偏宽松：「哪个区适合有小孩的家庭」没有预算也没有关键词，但明显在话题内，
必须受理。只有**完全没有房产信号**且**呈现明显离题句式**才拒。实测 16 条全对：

```
受理  预算 110 万，三房，北岸 / 哪个区适合有小孩的家庭 / 学区好的地方
      $1.2m 3 bed east auckland / which suburb suits a family on 1.5m
拒答  写一首关于奥克兰的诗 / 帮我写一段 python 代码 / 法国首都是哪里
      translate this to English / how do I center a div
```

注意「写一首关于奥克兰的诗」含"奥克兰"却被正确拒掉。
