...
const PLAN_TOPIC = /unitary\s+plan|planning\s+zone|mixed\s+housing|single\s+house\s+zone|terrace\s+housing|building\s+height|height\s+limit|how\s+(?:tall|high)\b.{0,40}\bbuild|recession\s+plane|subdivi(?:de|sion)|yard\s+(?:rule|requirement)|impervious|规划|分区|建筑高度|能建多高|限高|退界|细分|不透水|覆盖率|混合住宅(?:城市|郊区)区?|住宅混合(?:城市|郊区)区|独立住宅区|单户住宅区|排屋.{0,3}公寓楼区/i;
